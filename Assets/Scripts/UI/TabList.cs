...
public class TabList : MonoBehaviour
{
	...
	void SetActiveTab()
	{
		...
		foreach (GameObject tab in listTab)
		{
			...
			{
				tab.transform.Find("Panels").transform.gameObject.SetActive(false);

			}

			/// Activate child's "Button" on start
			if (tab.transform.Find("Button") != null)
			{
				tab.transform.Find("Button").transform.gameObject.SetActive(true);
